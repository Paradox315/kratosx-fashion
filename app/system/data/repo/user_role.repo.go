...
func (u *userRoleRepo) Select(ctx context.Context, id uint) (*model.UserRole, error) {
	...
	usr, err := ur.WithContext(ctx).Where(ur.ID.Eq(id)).Select(ur.ID, ur.UserID, ur.RoleID).First()
	if err != nil {
...
func (u *userRoleRepo) SelectAll(ctx context.Context) ([]*model.UserRole, error) {
	ur := u.baseRepo.UserRole
	urs, err := ur.WithContext(ctx).Select(ur.ID, ur.UserID, ur.RoleID).Find()
	if err != nil {
...
func (u *userRoleRepo) SelectAllByUserID(ctx context.Context, uid uint) ([]*model.UserRole, error) {
	ur := u.baseRepo.UserRole
	list, err := ur.WithContext(ctx).Where(ur.UserID.Eq(uid)).Select(ur.ID, ur.UserID, ur.RoleID).Find()
	if err != nil {
