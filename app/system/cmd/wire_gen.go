...
func initApp(confServer *conf.Server, registry *conf.Registry, storage *conf.Storage, confData *conf.Data, jwt *conf.JWT, logger *conf.Logger) (*kratos.App, func(), error) {
	...
	fiberMiddleware := middleware.NewJwtService(jwtRepo, userUsecase, client, logLogger)
	...
	publicUsecase := biz.NewPublicUsecase(userRepo, userRoleRepo, loginLogRepo, captchaRepo, jwtRepo, storageStorage, logLogger)
	...
	resourceRouterRepo := repo.NewResourceRouterRepo(dataData, confServer, logLogger, syncedEnforcer)
	resourceUsecase := biz.NewResourceUsecase(resourceMenuRepo, resourceRouterRepo, transaction, logLogger)
	resourceService := service.NewResourceService(resourceUsecase, logLogger)
	xhttpServer := server.NewHTTPServer(confServer, fiberMiddleware, pubService, userService, roleService, resourceService, logLogger)
