...
func (u *UserUsecase) Init(ctx context.Context) (user *pb.UserState, err error) {
	...
	user = &pb.UserState{
		...
		Creator:      upo.Creator,
		Status:       upo.Status == model.UserStatusNormal,
		RegisterDate: upo.CreatedAt.Format(timeFormat),
	}
	extras := upo.GetExtras()
	if extras != nil {
		user.Address = extras.Address
	...
	if err != nil {
		u.log.WithContext(ctx).Errorf("get user failed: %s", err.Error())
		err = api.ErrorUserFetchFail("用户角色关系不存在")
		return
	}
	for _, ur := range urs {
		user.Roles = append(user.Roles, uint64(ur.RoleID))
	}
	user.CurrentRole = lo.Ternary(user.Roles != nil && len(user.Roles) > 0, user.Roles[0], 0)
	return
}
