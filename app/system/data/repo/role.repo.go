...
func (r *RoleRepo) Select(ctx context.Context, id uint) (*model.Role, error) {
	rr := r.baseRepo.Role
	role, err := rr.WithContext(ctx).Where(rr.ID.Eq(id)).Select(rr.ALL).First()
	if err != nil {
...
func (r *RoleRepo) SelectByIDs(ctx context.Context, rids []uint) ([]*model.Role, error) {
	rr := r.baseRepo.Role
	roles, err := rr.WithContext(ctx).Where(rr.ID.In(rids...)).Select(rr.ALL).Find()
	if err != nil {
...
func (r *RoleRepo) SelectPage(ctx context.Context, limit, offset int) (roles []*model.Role, total int64, err error) {
	rr := r.baseRepo.Role
	roles, total, err = rr.WithContext(ctx).Select(rr.ALL).FindByPage(offset, limit)
	if err != nil {
