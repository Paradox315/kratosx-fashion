...
func (u *userRepo) Select(ctx context.Context, id uint) (*model.User, error) {
	result, err, _ := u.sf.Do(fmt.Sprintf(userKey, id), func() (*model.User, error) {
		...
		ur := u.baseRepo.User
		user, err := ur.WithContext(ctx).Where(ur.ID.Eq(id)).Select(ur.ALL).First()
		if err != nil {
...
func (u *userRepo) SelectByUsername(ctx context.Context, username string) (*model.User, error) {
	ur := u.baseRepo.User
	user, err := ur.WithContext(ctx).Where(ur.Username.Eq(username)).Select(ur.ALL).First()
	if err != nil {
...
func (u *userRepo) SelectPasswordByUID(ctx context.Context, uid uint) (*model.User, error) {
	ur := u.baseRepo.User
	user, err := ur.WithContext(ctx).Where(ur.ID.Eq(uid)).Select(ur.ALL).First()
	if err != nil {
...
func (u *userRepo) SelectByMobile(ctx context.Context, mobile string) (*model.User, error) {
	ur := u.baseRepo.User
	user, err := ur.WithContext(ctx).Where(ur.Mobile.Eq(mobile)).Select(ur.ALL).First()
	if err != nil {
...
func (u *userRepo) SelectByEmail(ctx context.Context, email string) (*model.User, error) {
	ur := u.baseRepo.User
	user, err := ur.WithContext(ctx).Where(ur.Email.Eq(email)).Select(ur.ALL).First()
	if err != nil {
